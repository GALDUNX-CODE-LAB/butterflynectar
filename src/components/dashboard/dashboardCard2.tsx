    ...
    <div className="order-3 lg:order-1 contained w-full overflow-hidden">
      ...
      <div className="order-2 lg:order-2  contained">
        <div className="flex items-center justify-between w w-full">
            <h3 className="w-[30%] text-shared">
                Activities
            </h3>
            <div className="rounded-3xl flex items-center bg-background w-[50%] text-[13px]">
            <button className="h-full w-[50%] text-sm py-2 px-1 focus:bg-[#5849EF] focus:text-white rounded-3xl">
                Refer
            </button>
            <button className="h-full py-2 px-1 w-[50%] text-sm  focus:bg-[#5849EF] focus:text-white rounded-3xl">
                Bookings
            </button>
        </div>
        </div>
        <div className="flex flex-col gap-3 items-center">
            ...
            {heading.map((x)=>(
                <h4 key={x.id} className="text-left text-[12px]">{x.name}</h4>
            ))}
        </div>
        <div className=" w-full mt-7">
            {avtivities.map((x)=>(
                <div className="grid grid-cols-4 gap-x-3 text-left text-[13px] text-shared" key={x.id}>
                     <h4 className="text-left mb-9">{x.name}</h4>
                      <h4 className="text-left">{x.date}</h4>
                      <h4 className="text-center">{x.refered}</h4>
                      <h4 className="text-left">{x.status}</h4>
                </div>
      ...
      <div className="order-1 lg:order-3 bg-white pt-4 px-4 rounded-2xl w-full">
        ...
        <div className="w-full grid grid-cols-3 gap-x-2 text-[13px] pt-3">
            
            <div className="flex items-center gap-1 "><input type="radio" className="accent-[#5849EF]" /> <span>bookings</span> </div>
             <div className="flex items-center gap-1" ><input type="radio" className="accent-[#CFEE49]" /> <span>Referrals</span> </div>
              <div className="flex items-center gap-1"><input type="radio" className="accent-[#CC1C43]" /> <span>Pending </span></div>
        </div>
        <ChartPieDonutText/>
      </div>
    </div>
