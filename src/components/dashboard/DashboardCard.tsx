...
const DashboardCard = () => {
  ...
  return (
    <div className="grid grid-cols-2  lg:grid-cols-4 gap-4 ">
      {data.map((x) => (
        <div className="bg-white pt-5 pb-2 px-3 rounded-2xl relative" key={x.id}>
          <h3 className="text-shared">{x.name}</h3>
          <div className="flex items-center gap-5 mt-3">
            <h1 className="text-3xl font-bold font-Ruska">{x.value}</h1>
            <span className="text-sm text-shared">{x.percentage}</span>
          </div>
